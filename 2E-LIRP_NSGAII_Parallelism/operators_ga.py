...
# funcion de ejecucion completa del algoritmo genetico
def run_ga(individuos, n_clientes, n_centroslocales, n_centrosregionales, n_periodos, n_productos, n_vehiculos_s, n_vehiculos_p, capacidad_cr, capacidad_cl, capacidad_vehiculos_p, capacidad_vehiculos_s, demanda_clientes, demand_cl_poblation, inventario, costo_instalaciones_cl, costo_instalaciones_cr, costo_compraproductos, costo_transporte, costo_inventario, costo_rutas_s, costo_rutas_p, costo_vehiculos_s, costo_vehiculos_p, costo_humano, n_generaciones, dominancias, distancias_t, demand_cr_poblation, Q_poblation, I_poblation, f1_poblation, f2_poblation, prob_mut, colector, idx_pa, timer):
    for _ in tqdm(range(n_generaciones)):
        # inicio operadores geneticos
        # seleccion de los padres
        idx_parents = selection_padres(len(individuos), dominancias, distancias_t)
        # cruce
        ...
        idx_poblation = reduction(len(individuos), frentes_dict, distancias_t)
        individuos = []
        demand_cr_poblation = []
        demand_cl_poblation = []
        Q_poblation = []
        ...
        dominancias, distancias_t, frentes_dict = frentes(len(individuos), f1_poblation, f2_poblation)

        # almacenaje de datos en el colector
        idx_temp = []
        ind_temp = []
        id_d = 0
        cr_dem_temp = []
        ...
        f2_temp = []
        dist_temp = {}
        for ind in frentes_dict[0]:
            idx_temp.append(ind)
            ind_temp.append(individuos[ind])
            cr_dem_temp.append(demand_cr_poblation[ind])
            cl_dem_temp.append(demand_cl_poblation[ind])
            q_temp.append(Q_poblation[ind])
            i_temp.append(I_poblation[ind])
            f1_temp.append(f1_poblation[ind])
            f2_temp.append(f2_poblation[ind])
            dist_temp[id_d] = distancias_t[ind]
            id_d += 1
        colector.value[idx_pa] = [ind_temp, cr_dem_temp, cl_dem_temp, q_temp, i_temp, f1_temp, f2_temp, dist_temp]
        time.sleep(timer)
        # intercambio de datos dentro del colector
        if idx_pa == 0:
            c_temp = colector.value[idx_pa]
            colector.value[idx_pa] = colector.value[1]
            colector.value[1] = c_temp
        else:
            c_temp = colector.value[idx_pa]
            colector.value[idx_pa] = colector.value[0]
            colector.value[0] = c_temp
        # reemplazo de datos
        replaced = []
        to_replace = []
        # revisar dominancia de todos los individuos del frente
        for ind_a in range(len(colector.value[idx_pa][5])):
            n = 0
            for k in range(len(individuos)):
                # condicion de concepto de dominancia
                if ((colector.value[idx_pa][5][ind_a] >= f1_poblation[k]) and (colector.value[idx_pa][6][ind_a] >= f2_poblation[k])) and ((colector.value[idx_pa][5][ind_a] > f1_poblation[k]) or (colector.value[idx_pa][6][ind_a] > f2_poblation[k])):
                    n += 1
                else:
            ...
            # si no es dominado
            if n == 0:
                to_replace.append(ind_a)
        # si el tamaño de los individuos no dominados es menor o igual al 10% de la poblacion
        if len(to_replace) <= int(0.1*len(individuos)):
            for id_tr in to_replace:
                # realiza el reemplazo
                id_r = np.random.randint(len(individuos))
                while id_r in replaced:
                    id_r = np.random.randint(len(individuos))
                individuos[id_r] = colector.value[idx_pa][0][id_tr]
                demand_cr_poblation[id_r] = colector.value[idx_pa][1][id_tr]
                demand_cl_poblation[id_r] = colector.value[idx_pa][2][id_tr]
                Q_poblation[id_r] = colector.value[idx_pa][3][id_tr]
                I_poblation[id_r] = colector.value[idx_pa][4][id_tr]
                f1_poblation[id_r] = colector.value[idx_pa][5][id_tr]
                f2_poblation[id_r] = colector.value[idx_pa][6][id_tr]
                replaced.append(id_r)
        else:  # si el tamaño de los individuos no dominados es mayor al 10% de la poblacion
            acept = []
            flag = 0
            dist_tr = {}
            replaced = []
            for id_trd in to_replace:
                dist_tr[id_trd] = colector.value[idx_pa][7][id_trd]
            dist_ord = sorted(dist_tr.items(), key=operator.itemgetter(1), reverse=True)  # ordenamos las distancias de apilamiento de mayor a menor
            while len(acept) < int(0.1*len(individuos)):
                acept.append(dist_ord[flag][0])
                flag += 1
            for ind_a in acept:
                # realiza el reemplazo
                id_r = np.random.randint(len(individuos))
                while id_r in replaced:
                    id_r = np.random.randint(len(individuos))
                individuos[id_r] = colector.value[idx_pa][0][ind_a]
                demand_cr_poblation[id_r] = colector.value[idx_pa][1][ind_a]
                demand_cl_poblation[id_r] = colector.value[idx_pa][2][ind_a]
                Q_poblation[id_r] = colector.value[idx_pa][3][ind_a]
                I_poblation[id_r] = colector.value[idx_pa][4][ind_a]
                f1_poblation[id_r] = colector.value[idx_pa][5][ind_a]
                f2_poblation[id_r] = colector.value[idx_pa][6][ind_a]
                replaced.append(id_r)
    # almacenaje de la mejor aproximacion de pareto para cada isla
    pareto = [individuos, demand_cr_poblation, demand_cl_poblation, Q_poblation, I_poblation, f1_poblation, f2_poblation]
    colector.value[2].append(pareto)

    return individuos, demand_cr_poblation, demand_cl_poblation, Q_poblation, I_poblation, f1_poblation, f2_poblation
...
def mutation(hijos, demandas_hijos, n_centrosregionales, capacidad_cr, n_periodos, n_productos, inventario, costo_instalaciones_cl, costo_instalaciones_cr, costo_compraproductos, costo_transporte, costo_inventario, costo_rutas_s, costo_rutas_p, n_centroslocales, costo_vehiculos_s, costo_vehiculos_p, costo_humano, Q_hijos, I_hijos, f1_hijos, f2_hijos, prob_mut):
    ...
    for _ in range(h_muts):
        ...
        if cr_mut != 0:
            ...
            # costos f2
            cost_sufr_hum = round(fitness_f2(hijo_copy[3], n_periodos, costo_humano, n_centroslocales), 3)
